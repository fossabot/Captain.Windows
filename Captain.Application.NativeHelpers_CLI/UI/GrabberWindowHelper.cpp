...
void GrabberWindowHelper::UpdateMonitorGeometryInfo()
{
  ...
  for (UINT uiAdapterIdx = 0; pFactory->EnumAdapters(uiAdapterIdx, &pAdapter) != DXGI_ERROR_NOT_FOUND; uiAdapterIdx++, pAdapter->Release())
  {
    ...
    for (UINT uiOutputIdx = 0; pAdapter->EnumOutputs(uiOutputIdx, &pOutput) != DXGI_ERROR_NOT_FOUND; uiOutputIdx++, pOutput->Release())
    {
      ...
      if (outputWindowRect.left < this->minLeft)
      {
        outputWindowRect.left = this->minLeft;
      }
      if (outputWindowRect.top < this->minTop)
      {
        outputWindowRect.top = this->minTop;
      }
      if (outputWindowRect.right > this->maxRight)
      {
        outputWindowRect.right = this->maxRight;
      }
      if (outputWindowRect.bottom > this->maxBottom)
      {
        outputWindowRect.bottom = this->maxBottom;
      }
...
IntPtr GrabberWindowHelper::WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, bool % handled)
{
  ...
  case WM_WINDOWPOSCHANGING:
  {
    ...
    if (pos->x < this->minLeft)
    {
      pos->x = this->minLeft;
    }
    if (pos->y < this->minTop)
    {
      pos->y = this->minTop;
    }
    if (pos->x + pos->cx > this->maxRight)
    {
      pos->x = this->maxRight - pos->cx;
    }
    if (pos->y + pos->cy > this->maxBottom)
    {
      pos->y = this->maxBottom - pos->cy;
    }
...
GrabberWindowHelper::~GrabberWindowHelper()
{
  ...
  if (this->prcOrgBounds)
  {
    delete this->prcOrgBounds;
  }
}
}
}
