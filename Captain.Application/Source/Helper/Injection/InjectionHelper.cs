...
namespace Captain.Application {
  ...
  internal static class InjectionHelper {
    ...
    internal static InjectionState Inject(int pid, string path32, string path64) {
      ...
      Log.WriteLine(LogLevel.Debug, $"handle for process {pid} opened successfully");

      // try to guess if this is a 32- or 64-bit process
      if (!Kernel32.IsWow64Process(processHandle, out bool isWowProcess)) {
        Log.WriteLine(LogLevel.Warning, "could not guess remote process architecture - trying with 32-bit library");
      }
      ...
      Log.WriteLine(LogLevel.Debug, $"library to be injected: {path}");
      ...
      Log.WriteLine(LogLevel.Debug,
        $"allocated {pathBufferLength} bytes in remote process memory (0x{remotePathBuffer.ToInt64():x8})");
      ...
      Log.WriteLine(LogLevel.Debug, $"wrote {pathBufferLength} bytes to remote process memory");
      Marshal.FreeHGlobal(localPathBuffer);
    ...
    internal static void Eject(InjectionState state) {
      ...
      // create remote thread for loading the library
      Log.WriteLine(LogLevel.Debug, "ejecting remote library");
      ...
      if (Kernel32.GetExitCodeThread(threadHandle, out int exitCode)) {
        if (exitCode == 1) { Log.WriteLine(LogLevel.Informational, "library ejection succeeded!"); } else {
          Log.WriteLine(LogLevel.Warning, "could not eject remote library!");
        }
      } else { Log.WriteLine(LogLevel.Warning, "library ejection status unknown"); }

      Log.WriteLine(LogLevel.Debug, "closing remote process handle");
      Kernel32.CloseHandle(state.RemoteProcessHandle);
    }
  }
}