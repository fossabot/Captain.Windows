...
namespace Captain.Application {
  ...
  internal sealed partial class ActionPropertiesDialog : Window {
    ...
    /// </summary>
    internal List<(string ActionType, object Options)> Actions { get; }

    /// <inheritdoc />
    /// <summary>
    ///   Class constructor
    /// </summary>
    public ActionPropertiesDialog(List<(string ActionType, object Options)> outputStreams = null) {
      InitializeComponent();
      ...
      Actions = outputStreams ?? new List<(string ActionType, object Options)>();
      UpdateList();
    }
    ...
    private void UpdateList() {
      ...
      this.streamListView.Items.AddRange(Actions.Select(o => {
        try {
          ...
          return new ListViewItem {
            Text = String.Format("Unknown Action ({0})", o.ActionType),
            Tag = null
          };
        }
      }).ToArray());
    ...
    private void OnAddActionLinkButtonClick(object sender, EventArgs eventArgs) {
      ...
      if (dialog.ShowDialog(this) == DialogResult.OK) {
        foreach (PluginObject stream in dialog.Streams) {
          Actions.Add((stream.Type.ToString(), null));
        }
        ...
        if (dialog.Streams.Count() == 1 && dialog.Streams.First().Configurable) {
          try {
            ...
            var configurableObject =
              FormatterServices.GetUninitializedObject(dialog.Streams.First().Type) as IHasOptions<object>;
    ...
    private void OnStreamListItemActivated(object sender, EventArgs eventArgs) {
      if (this.streamListView.SelectedItems[0].Tag is PluginObject pluginObject && pluginObject.Configurable) {
        try {
          ...
          var configurableObject =
            FormatterServices.GetUninitializedObject(pluginObject.Type) as IHasOptions<object>;
          configurableObject?.DisplayOptionsInterface(this);
        } catch (Exception exception) {
